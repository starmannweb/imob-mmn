        ...
        <div className="animate-in flex-1 flex flex-col w-full max-w-[1400px]">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <div className="flex bg-slate-100 dark:bg-slate-800/50 p-1 rounded-lg border border-slate-200 dark:border-slate-700/50 mr-2">
                        <Link href={`/painel/imoveis?tab=${currentTab}&view=grid`} className={`p-1.5 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-white text-slate-900 dark:bg-slate-700 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>
                        </Link>
                        <Link href={`/painel/imoveis?tab=${currentTab}&view=list`} className={`p-1.5 rounded-md transition-colors ${viewMode === 'list' ? 'bg-white text-slate-900 dark:bg-slate-700 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
                        </Link>
                    </div>
                    <button className="flex items-center justify-center gap-2 bg-transparent text-slate-500 hover:bg-slate-100 dark:text-slate-300 dark:hover:text-white dark:hover:bg-slate-800 border border-transparent px-4 py-2 rounded-lg text-sm font-medium transition-colors">
                                        ...
                                        <div className="flex-1 flex flex-col sm:flex-row w-full items-start sm:items-center justify-between gap-4">
                                            <div className="flex-1">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                     {prop.area && (
                                                         <div className="bg-slate-50 dark:bg-slate-900/80 text-slate-600 dark:text-slate-300 text-xs px-2.5 py-1 rounded-md border border-slate-100 dark:border-slate-800 font-bold whitespace-nowrap">
                                                             Área: {prop.area}m²
                                                         </div>
                                                     )}
                                                </div>
                                            ...
                                            <div className="flex flex-col sm:items-end w-full sm:w-auto">
                                                ...
                                                <div className="flex items-center gap-2 w-full sm:w-auto">
                                                    ...
                                                    <form action={async () => {
                                                        ...
                                                        <Button variant="ghost" size="sm" className="text-red-500 bg-red-50 hover:bg-red-100 dark:bg-red-900/10 dark:hover:bg-red-900/30 rounded-lg h-9 px-3">
                                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                                        </Button>
                                                    </form>
                                                </div>
